...
int force_nodma      = 0;
#ifdef __APPLE__    // Josh Gibson
unsigned long selected_fc      = 0;
unsigned long selected_window  = 0;
unsigned long selected_start   = 0;
...
{
#ifdef __TL_ARDUINO__
    tljtag_setup_cable_type(wiggler);
#else
#ifdef USE_JTMODS
...
void identify_flash_part(void)
{
    ...
    while (flash_chip->vendid)
    {
        if ((flash_chip->vendid == vendid) && (flash_chip->devid == devid))
        {
            ...
            {
                FLASH_MEMORY_START = (unsigned int)selected_window;
            }
            else
            {
                switch (proc_id)
...
//        break;
            ...
            if (strcasecmp(AREA_NAME,"CUSTOM")==0)
            {
                strcat(AREA_NAME,".BIN");
                FLASH_MEMORY_START = (unsigned int)selected_window;
                AREA_START         = (unsigned int)selected_start;
                AREA_LENGTH        = (unsigned int)selected_length;
            }
